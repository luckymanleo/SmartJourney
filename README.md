...
- **未登录保护**：PC/移动端受保护页面（行程、搜索、设置等）未登录时显示登录引导；PC 端弹出登录弹窗，移动端引导返回首页登录
- **多类型搜索**：机票、火车票、酒店、景点、美食、市内交通
- **AI 智能规划**：自然语言输入自动解析出发地/目的地/天数/人数/预算，LLM + MCP 生成完整行程，SSE 流式返回；支持往返行程（出发地⇄目的地）
...
- **行程地图导航**：高德地图集成，行程 POI 可视化、按天过滤、点击联动定位、"开始导航"唤起高德 App
  ...
  - 交通类站点/机场自动地理编码
  - **POI 照片自动富化**：行程生成时从高德获取照片存 DB（`trip_items.photos`），前端直读避免实时搜索错配；DB 无照片时回退坐标周边搜索
- **双端支持**：移动端 + PC Web（侧边栏布局），Vite 多页面构建
- **行程管理**：CRUD + 时间线视图（单价/总额显示）+ 行程简介 + 出行提示 + 预算概览（原预算对比）+ 天气集成 + 删除确认弹窗
...
- **照片**：行程生成时异步从高德获取照片存入 `trip_items.photos`（JSON），前端直读；无照片时用坐标周边搜索回退
- **匹配策略**：非交通类 POI 用 `poi/around` 坐标+关键词双约束搜索取代纯文字搜索，命中率显著提升
