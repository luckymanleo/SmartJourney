...
### 数据库连接池（.env + database.py）

| 参数 | 默认 | 说明 |
|------|------|------|
| DB_POOL_SIZE | 10 | 连接池基本连接数 |
| DB_MAX_OVERFLOW | 20 | 额外可创建连接数 |
| DB_POOL_RECYCLE | 1800 | 连接回收时间(秒) |

### PostgreSQL 服务端（docker-compose.yml）
...
- **gzip 压缩**：JS/CSS/JSON 压缩率 60-80%
- **静态资源缓存**：带 hash 文件永久缓存（immutable）
- **SSE 代理**：`proxy_buffering off`, `proxy_read_timeout 3600s`

### Redis 持久化（docker-compose.yml）
...
```sql
-- 行程过期查询优化
CREATE INDEX CONCURRENTLY ix_trips_status_end_date ON trips (status, end_date) WHERE status = 'active';
```
