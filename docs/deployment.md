...
> 详细 nginx 配置分析与数据流见 [Nginx 架构分析](nginx-architecture-analysis.md)
...
- [ ] uvicorn 多 worker（--workers 4，需真实 Redis 避免 MemoryRedis 不一致）
- [ ] 数据库部分索引（trips expiry 查询，`CREATE INDEX CONCURRENTLY`）
