...
export default function PlanPagePC() {
  ...
  // ── Fluid font sizes (viewport-based) ──
  const fs = {
  ...
  const formPanel = (
    <div className="space-y-4">
      ...
      <div>
        <label className="text-gray-600 mb-2 flex items-center gap-1" style={fs.label}>
        ...
        <input value={specialNotes} onChange={e => setSpecialNotes(e.target.value)}
          ...
          style={{...fs.input, padding: 'clamp(6px, 1vh, 10px) clamp(8px, 1.2vw, 12px)'}} />
      </div>
      <div>
      ...
      <button onClick={isPlanning ? cancelPlan : handleGenerate}
        ...
        className={`rounded-2xl py-3.5 font-semibold flex items-center justify-center gap-1.5 transition-colors shadow-sm whitespace-nowrap ${
          ...
          <div className="overflow-y-auto space-y-3 pr-3" style={{flex: '0 0 25%'}}>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                <div className="space-y-3">
                  {weatherData.origin_weather && (
                    <div>
                      <div className="font-semibold text-gray-700 mb-1.5" style={fs.small}>🌤️ {weatherData.origin}（出发地）</div>
                      <div className="text-gray-500 leading-relaxed whitespace-pre-line" style={fs.small}>{weatherData.origin_weather}</div>
                    </div>
                  )}
                  {weatherData.dest_weather && (
                    <div className="pt-3 border-t border-gray-100">
                      <div className="font-semibold text-gray-700 mb-1.5" style={fs.small}>🌤️ {weatherData.dest}（目的地）</div>
                      <div className="text-gray-500 leading-relaxed whitespace-pre-line" style={fs.small}>{weatherData.dest_weather}</div>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => navigate('/trips')}
                className="flex-1 border border-primary-300 text-primary-600 rounded-lg font-medium hover:bg-primary-50 transition-colors text-center"
                style={{...fs.small, padding: 'clamp(6px, 1vh, 10px) clamp(8px, 1vw, 12px)'}}>
                我的行程
              </button>
              <button onClick={() => usePlanStore.setState({ tripData: null, tripRoutes: [], steps: [] })}
                className="flex-1 border border-gray-300 text-gray-600 rounded-lg font-medium hover:bg-gray-100 transition-colors text-center"
                style={{...fs.small, padding: 'clamp(6px, 1vh, 10px) clamp(8px, 1vw, 12px)'}}>
                重新规划
              </button>
            </div>
